---
statistics: true
comments: true
icon: material/home
hide:
    - statistics
---

<br/>

<h1 align="center">HDpip</h1>

<p align="center"><img alt="logo" src="./logo.png" /></p>

<p align="center">
一个基于 <code>maliang</code> 的pip GUI
</p>

<p align="center">
<a href="https://github.com/handongliren/HDpip/releases"><img alt="版本" title="版本" src="https://img.shields.io/github/v/release/handongliren/HDpip?include_prereleases&label=%e7%89%88%e6%9c%ac&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://pypistats.org/packages/HDpip"><img alt="下载" title="下载" src="https://img.shields.io/pypi/dm/HDpip?label=%e4%b8%8b%e8%bd%bd&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://github.com/handongliren/HDpip/actions"><img alt="检查和测试" title="检查和测试" src="https://img.shields.io/github/actions/workflow/status/handongliren/HDpip/python-package.yml?label=%e6%a3%80%e6%9f%a5%e5%92%8c%e6%b5%8b%e8%af%95&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://codecov.io/gh/handongliren/HDpip"><img alt="代码覆盖率" title="代码覆盖率" src="https://img.shields.io/codecov/c/github/handongliren/HDpip?label=%e4%bb%a3%e7%a0%81%e8%a6%86%e7%9b%96%e7%8e%87&logoColor=white&logo=codecov" /></a>
<br/>
<a href="https://github.com/handongliren/HDpip/watchers"><img alt="关注" title="关注" src="https://img.shields.io/github/watchers/handongliren/HDpip?label=%e5%85%b3%e6%b3%a8&style=flat&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://github.com/handongliren/HDpip/forks"><img alt="复刻" title="复刻" src="https://img.shields.io/github/forks/handongliren/HDpip?label=%e5%a4%8d%e5%88%bb&style=flat&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://github.com/handongliren/HDpip/stargazers"><img alt="星标" title="星标" src="https://img.shields.io/github/stars/handongliren/HDpip?label=%e6%98%9f%e6%a0%87&color=gold&style=flat&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://github.com/handongliren/HDpip/issues"><img alt="议题" title="议题" src="https://img.shields.io/github/issues/handongliren/HDpip?label=%e8%ae%ae%e9%a2%98&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://github.com/handongliren/HDpip/pulls"><img alt="拉取请求" title="拉取请求" src="https://img.shields.io/github/issues-pr/handongliren/HDpip?label=%e6%8b%89%e5%8f%96%e8%af%b7%e6%b1%82&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
<a href="https://github.com/handongliren/HDpip/discussions"><img alt="讨论" title="讨论" src="https://img.shields.io/github/discussions/handongliren/HDpip?label=%e8%ae%a8%e8%ae%ba&logo=data:image/svg+xml;charset=utf-8;base64," /></a>
</p>

<p align="center">
<a href="https://github.com/handongliren/HDpip/pulse"><img alt="Insights" src="https://repobeats.axiom.co/api/embed/1e7c5b49fa1d2160b0ebc5231f659e8f99bdc9d2.svg" /></a>
</p>

<p align="center" markdown>
![light](https://api.star-history.com/svg?repos=handongliren/HDpip&type=Date&theme=light#only-light)
![dark](https://api.star-history.com/svg?repos=handongliren/HDpip&type=Date&theme=dark#only-dark)
</p>

## 📦 安装

要安装该包，你的 Python 版本必须 **`>=3.10`**。用以下命令进行安装：

```shell linenums="0"
pip install HDpip
```

### 🛠️ 依赖包

* <https://github.com/pypa/pip>: 都知道是啥

* <https://github.com/Xiaokang2022/maliang>: GUI框架

* <https://github.com/yaml/pyyaml>: `.yaml`文件操作支持

* <https://github.com/tox-dev/pipdeptree>: pip依赖树

## 👀 更多

### 🔗 链接

这里是一些可能对您有帮助的链接：

<div class="grid cards" markdown>

* [📑 **开源许可**](./LICENSE.md)

    ***

    是的，GPL-3……

* [📘 **更新日志**](./CHANGELOG.md)

    ***

    所有版本的更新日志

* [🚀 **Gitee 仓库镜像源**](https://gitee.com/handongliren/HDpip){ target="_black" }

    ***

    ![Gitee Stars](https://gitee.com/handongliren/HDpip/badge/star.svg?theme=dark#only-dark "Gitee Stars")
    ![Gitee Stars](https://gitee.com/handongliren/HDpip/badge/star.svg?theme=white#only-light "Gitee Stars")
    ![Gitee Forks](https://gitee.com/handongliren/HDpip/badge/fork.svg?theme=dark#only-dark "Gitee Forks")
    ![Gitee Forks](https://gitee.com/handongliren/HDpip/badge/fork.svg?theme=white#only-light "Gitee Forks")

</div>

!!! quote ""

    <div markdown align="center">
    本站共有 {{ pages }} 个页面，{{ words }} 个字，{{ codes }} 行代码，{{ images }} 张图片，您是第 <span id="busuanzi_site_uv"><i class="fa fa-spinner fa-spin"></i></span> 位访客！
    </div>

<div align="center" markdown>
[✨最新内容](./CHANGELOG.md){ .md-button .md-button--primary title="点我了解最近更新的内容！" }
[<big>⭐支持本项目</big>](https://github.com/handongliren/HDpip/){ .md-button .md-button--primary title="给项目点个 Star 吧，球球了！" }
</div>
