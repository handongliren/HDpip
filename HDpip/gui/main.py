...
class ControlCanvas(custom.containers.Canvas):
    ...
    def __init__(self, master: maliang.Canvas | maliang.core.virtual.Widget | maliang.Tk | maliang.Toplevel, data_manager: core.data.DataManager = core.data.data_manager):
        ...
        self.install_button = custom.widgets.Button(self, ss((40, 40)), ss((120, 60)), theme = "outline-success", text = self.data_manager.language["main"]["install_button"], icon = bi("box-seam"), fontsize = ss(30))
        self.uninstall_button = custom.widgets.Button(self, ss((190, 40)), ss((120, 60)), theme = "outline-danger", text = self.data_manager.language["main"]["uninstall_button"], icon = bi("trash"), fontsize = ss(30))
        self.upgrade_button = custom.widgets.Button(self, ss((40, 140)), ss((120, 60)), theme = "outline-primary", text = self.data_manager.language["main"]["upgrade_button"], icon = bi("arrow-up"), fontsize = ss(30))
        self.setting_button = custom.widgets.Button(self, ss((190, 140)), ss((120, 60)), theme = "outline-secondary", text = self.data_manager.language["main"]["setting_button"], icon = bi("gear"), fontsize = ss(30))
...
class Main(custom.containers.Tk):
    ...
    def __init__(self, data_manager: core.data.DataManager = core.data.data_manager):
        ...
        self.about_canvas.create_line(ss((0, 149, 350, 149)), width = 1, fill = custom.color.gray_500)
        ...
        self.control_canvas.create_line(ss((0, 240, 350, 240)), width = 1, fill = custom.color.gray_500)
        self.base_canvas.create_line(ss((0, 390, 350, 390)), width = 1, fill = custom.color.gray_500)

        self.base_canvas.create_line(ss((350, 0, 350, 800)), width = 1, fill = custom.color.gray_500)
        self.base_canvas.create_line(ss((700, 0, 700, 800)), width = 1, fill = custom.color.gray_500)
        self.base_canvas.create_line(ss((0, 540, 350, 540)), width = 1, fill = custom.color.gray_500)
        self.base_canvas.create_line(ss((700, 400, 1200, 400)), width = 1, fill = custom.color.gray_500)
        self.renderLanguage()
